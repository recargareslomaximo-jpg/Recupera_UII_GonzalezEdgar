...
export default function TaskListPage({ tasks, onToggle, onDelete }) {
  ...
  // useEffect: Actualiza el título del navegador según las tareas pendientes
  useEffect(() => {
    document.title =
      pendingCount > 0 ? `(${pendingCount}) Mis Chambas` : "Mis Chambas";
  }, [pendingCount]);

  // Lista de filtros como datos (evita repetición)
  const filterOptions = [
    { key: "all", label: "Todas las chambas" },
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        ...
        {/* Encabezado */}
        <div>
          <h1 className="text-4xl font-bold text-app-text mb-2 transition-colors tracking-tight">
            Mis PapuTareas
          </h1>
        ...
        {tasks.length > 0 && (
          <div className="flex gap-3">
            {[
              ...
              <div
                key={stat.label}
                className={`rounded-xl border px-4 py-2 text-center transition-colors ${stat.color}`}
              >
      ...
      {/* Filtros — CORRECCIÓN: ahora usan el componente Button en lugar de <button> nativo */}
      {tasks.length > 0 && (
      ...
      {/* Lista de tareas o estado vacío — renderizado condicional */}
      {tasks.length === 0 ? (
        <EmptyState
          icon="🗒️"
